...
class OllamaLocal(LM):
    ...
    def basic_request(self, prompt: str, **kwargs):
        ...
        for i in range(kwargs["n"]):
            response = cached_ollama_request(url=urlstr, settings_dict_str=json.dumps(settings_dict), timeout=self.timeout_s)
            # response = requests.post(url=urlstr, json=settings_dict, timeout=self.timeout_s)
...
def cached_ollama_request_wrapped(url, settings_dict_str, timeout):
    return _cached_ollama_request(url=url, settings_dict=json.loads(settings_dict_str), timeout=timeout)

def _cached_ollama_request(url, settings_dict, timeout):
    # settings_tuple = dict_to_tuple(settings_dict)
    return requests.post(url=url, json=settings_dict, timeout=timeout)

cached_ollama_request = cached_ollama_request_wrapped